...
if ( ($func == 'add' || $func == 'edit') && rex::getUser()->isAdmin() ) {
  ...
  $xform = new rex_xform;
  // $xform->setDebug(TRUE);
  $xform->setHiddenField('page', rex_be_controller::getCurrentPage());
  ...
  if ($xform->objparams['form_show']) {
    ...
    echo $form;
    echo '</div></div>';
    echo '<br />&nbsp;<br /><table cellpadding="5" class="rex-table"><tr><td><a href="' . rex_url::currentBackendPage() . '"><b>&laquo; ' . rex_i18n::msg('xform_back_to_overview') . '</b></a></td></tr></table>';
    $show_list = false;
  } else {
    if ($func == 'edit') {
...
if ($show_list && rex::getUser()->isAdmin()) {
  ...
  echo '<table cellpadding=5 class=rex-table><tr><td>
    <a href=' . rex_url::currentBackendPage(array('func' => 'add')) . '><b>+ ' . rex_i18n::msg('xform_manager_table_add') . '</b></a>
    <!-- |  <a href=' . rex_url::currentBackendPage(array('func' => 'table_import')) . '><b>' . rex_i18n::msg('xform_manager_table_import') . '</b></a> -->
  ...
  $list->setColumnParams(rex_i18n::msg('edit'), array('page' => 'xform/manager', 'tripage' => 'table_field', 'table_name' => '###table_name###'));
  ...
  if (is_array($tables)) {
    foreach ($tables as $table) {
      ...
      if ($table['status'] == 1 && $table['hidden'] != 1 && rex::getUser() && (rex::getUser()->isAdmin() || rex::getUser()->hasPerm($table_perm))) {
        echo '<li><a href="' . rex_url::backendPage('xform_table_' . $table['table_name']) . '">' . $table['name'] . '</a></li>';
      }
    }
  }
